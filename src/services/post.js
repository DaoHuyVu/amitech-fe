...
export const searchPosts = async (s,size,page=1) => {
    let response = await handleGetMethod(() => axios.get(`/posts?filters[postTitle][$containsi]=${s}&fields[0]=slug&fields[1]=postTitle&populate[profile][fields][0]=url&populate[navigations][fields][1]=slug&pagination[pageSize]=${size}&pagination[page]=${page}`))
    return response.data
}